...
class BasketMoveSystem extends System {
  ...
  execute(delta, time) {
    this.queries.playerBaskets.results.forEach((entity) => {
      ...
      if (
        controlComponent.input.left &&
        leftMoveX >= controlComponent.boundries.minX
      ) {
      ...
      if (
        controlComponent.input.right &&
        righttMoveX <= controlComponent.boundries.maxX
      ) {
        gsap.to(basket.position, {
          x: righttMoveX,
          y: 1.35,
  ...
  execute(delta, time) {
    ...
    this.queries.movingObjects.results.forEach((entity) => {
      ...
      const controlComponent =
        basketEntity.getMutableComponent(ControllableBasket);

      const velocity =
        (movingComponent.velocity + movingComponent.acceleration * time) *
        delta;
      // console.log( velocity );
      obj.position.add(
        this.vec3.copy(movingComponent.direction).multiplyScalar(velocity)
      );
      ...
      if (obj.position.y < movingComponent.boundries.min.y) {
        obj.position.set(
          ...
          movingComponent.boundries.max.y +
            Math.random() * (movingComponent.respawnRange + time),
          0
        );
        // Remove a heart
        if (entity.getComponent(Egg).points >= 0) {
          ...
          heartsContainer.remove(
            heartsContainer.children[heartsContainer.children.length - 1]
          );

          if (controlComponent.lives <= 0) {
            // check for game over
    ...
    this.queries.eggs.results.forEach((entity) => {
      ...
      if (egg.position.distanceTo(basket.position) < 0.75) {
        ...
        document.getElementsByClassName("score")[0].textContent =
          controlComponent.score + " punktów";

        if (eggComponent.points < 0) {
          // black egg
          controlComponent.lives--;
          const heartsContainer = heartsEntity.getObject3D();
          heartsContainer.remove(
...
class CloudFloatingSystem extends System {
  ...
  execute(delta, time) {
    this.queries.clouds.results.forEach((entity) => {
      ...
      obj.position.add(
        this.vec3.copy(cloudComponent.direction).multiplyScalar(velocity)
      );

      if (obj.position.x > cloudComponent.boundries.max.x) {
        // Reached boundry, transform back to beginning (scrolling)

        obj.position.x =
          cloudComponent.boundries.min.x -
          cloudComponent.respawnRange * Math.random();
      }
    });
  }
}
